...
\ssh{Market for reserves and Federal funds rate}
...
\ssh{Market Equilibrium}
\includegraphics[scale=0.4]{./imgs/161.jpg}

\vspace{6pt}
\ssh{Response to open market operations}
...
\ssh{Response to change in interest rate on reserves}
...
\ssh{Conventional monetary policy tools}
\rn{1} Open market operations
...
\rn{2} Discount policy or discount window. 
...
\ssh{Relative Advantages of the Different Tools.}
\ra Open market operations are the most important because
\rn{1} Complete control over volume unlike discount operations where Fed cannot control the volume.
\rn{2} They are flexible and precise. Can be used for small or large operations.
\rn{3} They are easily reserved. To many purchases and be undo with immediate sales.
\rn{4} They can be implemented quickly and involve no administration delays. Changes to reserve requirements take time and are costly to implement.
...
\ssh{A framework for monetary policy in South Africa}
...
\ssh{How monetary policy is applied in South Africa}
\de{Open-market operations }
...
\ssh{Explain whether the formula M=m.MB applies to South Africa.}
...
\de{Government budget constraint}
...
\ssh{Liquidity preference theory}
\de{Motives for the demand for money}
\rn{1} Transactions Motive. Medium of exchange, demand for money declining due to new technology. Proportional to income.
\rn{2} Precautionary Motive. Cushion against unexpected wants. Proportional to income.
\rn{3} Speculative Motive. Store of wealth. Proportional to opportunity cost of holding which the interest earned on bonds.
...
\note{read study guide}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\sh{Chapter 24: Monetary Policy Theory}

\ssh{Response of monetary policy to shocks}
...
\ssh{How actively should policy-makers try to stabilize economic activity?}
\de{Lags and Policy Implementation}
\rn{1} Data lag
\rn{2} Recognition lag
\rn{3} Legislative lag
\rn{4} Implementation lag
\rn{5} Effectiveness lag


\ssh{Inflation: Always and everywhere a monetary phenomenon}
...
\ssh{Causes of inflationary monetary policy}
\rn{1} High employment targets and inflation
...
\ssh{Definition and measurement of inflation}
...
\ssh{Social conflict and inflation proneness}
...
\ra Three factors:
\rn{1} Regular significant increases in the price of goods or services which are an important input into the production process or of the cost of living
\rn{2} Is affected by the competitiveness of both its goods and labour markets. (powerful interest groups)
\rn{3} If the money stock is highly elastic. When there is less money and the demand for goods is scarcer, firms are encouraged to absorb more cost increases and accept lower profit margins.
\rn{4} An increase in the rand price of imported goods (for an unchanged volume) raises foreign claims on the local social product. South Africans have no choice but to give up some of their real income. This necessary and unavoidable real income sacrifice could be extremely painful.
\ra 3 ways to bear the pain.
\rn{1} Non-inflationary scenario,
\rna local producers and retailers keep their rand prices unchanged and accept lower profit margins.
\rn{2} Fully inflationary scenario (Inflationary spiral)
...
\rn{3} Partially inflationary scenario
...
\rn{2} A voluntary social contract between business, labour and government
...
\rn{3} Tight monetary policy: increasing the scarcity of demand
...
\rn{4} The role of the foreign sector
...
\rn{5} The self-reinforcing nature of lower inflation: the role of inflationary expectations
...
\rn{1} Contributes towards a general climate of instability and pessimism
\rn{2} Inflation means that changes in nominal prices no longer reflect changes in relative prices
\rn{3} Causes people to divert their effort and capital away from productive enterprise towards non-productive investment such collection stamps.
\ra Adverse effects on income distribution:
\rn{1} Those who lack the bargaining power to increase their nominal incomes
...
\rn{2} Those people who hold money (wallets and bank accounts)
...
\rn{3} Creditors – people who have lent money to others
...
\ssh{Discretion and the time-inconsistency problem.}
...
\ssh{Benefits of a credible nominal anchor.}
\rn{1} Has elements of a behaviour rule
\rna Helps over come the time inconsistency problem. 
\rna Allows the public to keep policy makers in check.
\rn{2} Helps anchor inflation expectations
...
\ssh{Transmission mechanism of monetary policy}
\ra The way in which monetary policy affects aggregate demand and the economy.

\ssh{Traditional interest rate channels}
...
\ssh{Other asset price channels}
\de{Exchange rate effects on net exports}
...
\vspace{6pt}
\de{Unanticipated Price level change}
...
\vspace{6pt}
\de{Household liquidity effects}
...
\ssh{Why are credit channels likely to be important?}
\rn{1} Large body of evidence supports the view that financial frictions of the type crucial to the operation of credit channels do affect firms employment and spending decisions.
\rn{2} Small firms are hurt more by tight monetary policy than large firms which are unlikely to be credit constrained.
\rn{3} Asymmetric information view has proved useful in explaining other important phenomenon like why financial intermediaries exist.
